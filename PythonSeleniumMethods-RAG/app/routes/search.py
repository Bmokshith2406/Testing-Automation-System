...
async def search_methods(payload: SearchRequest = Body(...)):
    ...
    cache_key = f"{raw_query}::rank={ranking_variant}"
    ...
    for rank, c in enumerate(final_list, start=1):
        try:
            ...
            md = payload_doc.get("method_documentation", {})

            response_items.append(
                SearchResultItem(
                    id=str(payload_doc["_id"]),
