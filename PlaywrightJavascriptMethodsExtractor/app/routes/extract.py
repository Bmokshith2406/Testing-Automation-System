...
@router.post("/", summary="Extract Playwright JavaScript methods and return CSV")
async def extract_methods(
    ...
    if not file and (not script or script.strip() == ""):
        raise HTTPException(
            status_code=400,
            detail="Provide either a JavaScript file upload or a 'script' text field."
        )

    try:
        # --------------------------------------------------
        # Load Playwright JavaScript source
        # --------------------------------------------------
        if file:
        ...
        # --------------------------------------------------
        # Store raw script for audit/debugging
        # --------------------------------------------------
        storage_error = None
        try:
        ...
        # --------------------------------------------------
        # AST-based Playwright JS method extraction
        # --------------------------------------------------
        ...
        download_name = f"playwright_js_methods_{timestamp.strftime('%Y%m%dT%H%M%SZ')}.csv"
        ...
        return JSONResponse(
            status_code=500,
            content={
                "detail": "Internal server error during Playwright JavaScript method extraction."
            }
        )
