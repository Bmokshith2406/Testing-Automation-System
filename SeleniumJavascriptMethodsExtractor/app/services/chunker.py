...
class MethodInfoLike(Protocol):
    """
    Protocol for extracted JavaScript method/function objects.
    ...
    - is_nested
    - injected_vars (list of strings: global variables, constructor variables, etc.)
    """
    name: str
    start_line: int
    end_line: int
...
class Chunk:
    """
    Represents a group of JavaScript methods grouped together.
    Used for batching, size-limiting, and downstream processing.
    """
    index: int
...
def _estimate_method_size(m: MethodInfoLike, buffer: int = 200) -> int:
    """
    Rough approximation of a JavaScript method/function size.
    Includes:
    - method/function code
    - injected variable lines
    - a small buffer for formatting
    """
...
def build_chunks(methods: List[MethodInfoLike], max_chars_per_chunk: int = 20000) -> List[Chunk]:
    """
    Build sequential chunks of JavaScript methods/functions where each chunk
    does not exceed max_chars_per_chunk.

    Ensures:
    - methods remain in original file order
    ...
    for m in methods:
        ...
        if current and would_exceed:
            chunks.append(Chunk(index=idx, methods=current,
                                total_chars=current_chars, total_methods=len(current)))
            idx += 1
            current = []
            current_chars = 0

        if not current and m_size > max_chars_per_chunk:
            chunks.append(Chunk(index=idx, methods=[m],
                                total_chars=m_size, total_methods=1))
            idx += 1
            continue
...
def chunk_to_text(chunk: Chunk, separator: str = "\n\n") -> str:
    """
    Convert a chunk of JavaScript methods into a merged text block.
    """
    parts: List[str] = []
...
def iter_chunk_texts(methods: List[MethodInfoLike], max_chars_per_chunk: int = 20000) -> Iterator[Tuple[int, str]]:
    """
    Yield (chunk_index, chunk_text) pairs for downstream use.
    """
...
def process_chunks_parallel(
    ...
    """
    Parallel processing helper for chunk operations.
    Useful for large JS codebases or multi-model extraction pipelines.
    """
...
def approximate_total_size(methods: List[MethodInfoLike]) -> int:
    """
    Compute total estimated size of all extracted JavaScript methods/functions.
    """
