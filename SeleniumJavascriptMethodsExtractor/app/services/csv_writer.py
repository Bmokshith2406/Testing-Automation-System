...
def prevent_excel_injection(text: str) -> str:
    """
    Prevent Excel from interpreting JavaScript code lines as formulas.
    """
...
def method_to_string(m: MethodInfo) -> str:
    """
    Convert a JavaScript method/function (with injected variables)
    into a normalized multiline text block.
    """
    parts: List[str] = []

    # Injected variables (globals, constructor vars, etc.)
    if m.injected_vars:
...
def write_methods_to_csv(methods: List[MethodInfo]) -> bytes:
    """
    Write extracted JavaScript methods/functions into a CSV file.
    One column only: "Raw Method".
    """
...
def iter_csv_rows(methods: List[MethodInfo]) -> Iterable[str]:
    """
    Generate CSV rows on-demand (streaming) for large JavaScript files.
    """
    yield "Raw Method\n"
