...
def measure_time(func: Callable) -> Callable:
    ...
    Used across the extraction pipeline (JS scanner, chunker, CSV writer).
    """
    @wraps(func)
...
def safe_decode(raw_bytes: bytes) -> str:
    """
    Safely decode uploaded script files.
    JavaScript Selenium files may contain UTF-8 or legacy encodings.
    """
    try:
...
def flatten(list_of_lists: Iterable[Iterable[Any]]) -> List[Any]:
    """
    Flatten nested lists. Used when combining extracted JS method blocks.
    """
